...
export async function POST(_req: NextRequest, ctx: RouteContext<"/api/mm/purchase-requisitions/[id]/approve">) {
  try {
    ...
    if (pr.status !== "DRAFT" && pr.status !== "SUBMITTED") {
      return Response.json({ error: "Only DRAFT or SUBMITTED PRs can be approved" }, { status: 400 });
    }
